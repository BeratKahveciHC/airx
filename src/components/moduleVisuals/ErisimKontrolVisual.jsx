...
export default function ErisimKontrolVisual({ accent = '#0ea5e9', inline, isTr = true }) {
  return (
    <VisualSection accent={accent} inline={inline}>
      ...
      >
        <BrowserMockup url="erisim-yetkisi" activeNav="Modüller" isTr={isTr}>
          <div style={{ background: '#f1f5f9', padding: '20px', minHeight: 340 }}>
            <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,30,80,0.06)' }}>
              <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #f1f5f9' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{isTr ? 'Erişim Yetkisi' : 'Access Permission'}</span>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10 }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    {(isTr ? ['Adı Soyadı', 'Cihazlar', 'İşlemler'] : ['Full Name', 'Devices', 'Actions']).map((col) => (
                      <th key={col} style={{ padding: '8px 14px', fontWeight: 700, color: '#475569', textAlign: (isTr ? col === 'İşlemler' : col === 'Actions') ? 'right' : 'left', fontSize: 10 }}>{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
