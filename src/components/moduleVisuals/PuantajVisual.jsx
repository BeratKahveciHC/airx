...
const SUMMARY_COLS_EN = [
  { label: 'Unknown', color: '#1e293b' },
  { label: 'Present', color: '#22c55e' },
  { label: 'Absent', color: '#f87171' },
  { label: 'Paid\nLeave', color: '#a78bfa' },
  { label: 'No\nOvertime', color: '#94a3b8' },
  { label: 'Late\nArrival', color: '#fb923c' },
  { label: 'Late\nExit', color: '#f97316' },
  { label: 'Early\nArrival', color: '#84cc16' },
  { label: 'Early\nExit', color: '#ef4444' },
  { label: 'Overtime', color: '#8b5cf6' },
  { label: 'Unpaid\nLeave', color: '#14b8a6' },
  { label: 'Public\nHoliday', color: '#38bdf8' },
  { label: 'Admin\nHoliday', color: '#3b82f6' },
  { label: 'Weekend', color: '#93c5fd' },
  { label: 'Contract\nHoliday', color: '#0ea5e9' },
]
...
export default function PuantajVisual({ accent = '#0ea5e9', inline, isTr = true }) {
  const SUMMARY_COLS = isTr ? SUMMARY_COLS_TR : SUMMARY_COLS_EN
  const filterFields = isTr
    ? [{ label: 'Yıl Seçiniz', value: '2026' }, { label: 'Ay Seçiniz', value: '4' }, { label: 'Şube', value: 'İkitelli Merkez' }]
    : [{ label: 'Select Year', value: '2026' }, { label: 'Select Month', value: '4' }, { label: 'Branch', value: 'İkitelli Merkez' }]
  return (
    <VisualSection accent={accent} inline={inline}>
      ...
      >
        <BrowserMockup url="puantaj-raporlari" activeNav="Raporlar" maxWidth={960} isTr={isTr}>
          <div style={{ background: '#f1f5f9', padding: '14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              <div style={{ padding: '10px 14px 8px', borderBottom: '1px solid #f1f5f9' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#1e293b' }}>{isTr ? '2026 Nisan Puantaj Raporları' : '2026 April Timesheet Reports'}</span>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', fontSize: 9 }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                      <th style={{ padding: '6px 10px', fontWeight: 700, color: '#475569', textAlign: 'left', minWidth: 110 }}>{isTr ? 'Adı Soyadı' : 'Full Name'}</th>
                      {DAYS.map(d => (
            ...
            >
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', fontSize: 8.5 }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                      <th style={{ padding: '6px 10px', fontWeight: 700, color: '#475569', textAlign: 'left', minWidth: 100 }}>{isTr ? 'Adı Soyadı' : 'Full Name'}</th>
